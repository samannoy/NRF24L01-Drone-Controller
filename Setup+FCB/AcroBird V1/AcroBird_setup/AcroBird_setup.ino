...
// This section describes the vehicle (this ID is JD83), useful for nrf encryption
...
// Change these to volatile int if setup stops at receiver input and comment out the following
...
byte channel_1_assign, channel_2_assign, channel_3_assign, channel_4_assign,channel_number_ok;
...
void loop(){
  ...
  if(error == 0){
    ...
    Serial.print(F("Channel 1 or Ailerons or Roll = "));
    Serial.println(receiver_input_channel_1);
    Serial.print(F("Channel 2 or Elevator or Pitch = "));
    Serial.println(receiver_input_channel_2);
    Serial.print(F("Channel 3 or Throttle = "));
    Serial.println(receiver_input_channel_3);
    Serial.print(F("Channel 4 or Rudder or Yaw = "));
  ...
  if(error == 0){  
    ...
    //Check for aileron/Roll movement
    check_inputs(1);
    wait_sticks_zero();
  }
  if(error == 0){
    ...
    //Check for Elevator/pitch movement
    check_inputs(2);
    wait_sticks_zero();
  }
  if(error == 0){
    ...
    //Check for Rudder/Yaw movement
    check_inputs(4);
    wait_sticks_zero();
  }

  ////The following are not needed as the controller is an analog joystick where the endpoints are fixed and known.
  /*if(error == 0){
    Serial.println(F(""));
    Serial.println(F(""));
 ...
 /*   if(high_channel_1 != ((EEPROM.read(9) << 8) | EEPROM.read(8)))error = 1;
  ...
  if(error == 0){
    ...
    Serial.println(F("You can now calibrate the esc's and upload the AcroBird code."));
  }
  else{
   Serial.println(F("The setup is aborted due to an error."));
   Serial.println(F("Check the connections and restart from the beginning"));
   Serial.println(F("Thank you for your patience"));
  }
  while(1);
}
...
//Check if a receiver input value is changing within 10 seconds

void check_inputs(byte movement)
{ 
  byte trigger = 0;
  timer = millis() + 10000;
  while(timer > millis() && trigger == 0){
    receive_input_NRF();
    delay(250);
    if(receiver_input_channel_1 > 220 || receiver_input_channel_1 < 35){
      trigger = 1;
      receiver_check_byte |= 0b00000001;
    }
    if(receiver_input_channel_2 > 220 || receiver_input_channel_2 < 35){
      trigger = 2;
      receiver_check_byte |= 0b00000010;
    }
    if(receiver_input_channel_3 > 220 || receiver_input_channel_3 < 35){
      trigger = 3;
      receiver_check_byte |= 0b00000100;
    }
    if(receiver_input_channel_4 > 220 || receiver_input_channel_4 < 35){
      trigger = 4;
  ...
  if(trigger == 0){
    error = 1;
    Serial.println(F("No stick movement detected in the last 10 seconds!!! (ERROR 2)"));
  }
  //Assign the stick to the function.
  else{
    if(movement == 1){
      if(trigger == 1){
      Serial.print(F("Roll is verified "));  }
      else{
        Serial.print(F("Wrong stick movement!!"));
        }
    }
    if(movement == 2){
      if(trigger == 2){
      Serial.print(F("Pitch is verified "));  }
      else{
        Serial.print(F("Wrong stick movement!!"));
        }
    }
    if(movement == 3){
      if(trigger == 3){
      Serial.print(F("Throttle channel is verified "));  }
      else{
        Serial.print(F("Wrong stick movement!!"));
        }
    }
    if(movement == 4){
      if(trigger == 4){
      Serial.print(F("Yaw is verified "));  }
      else{
        Serial.print(F("Wrong stick movement!!"));
        }
    }
...
//to be removed
/*void check_inputs(byte channel_number)
{
  receive_input_NRF();
  int channel_number_ok=0;
  timer = millis() + 10000;
while(timer > millis())
{
  delay(250);
 if (channel_number==1 )
 {
  if(receiver_input_channel_1>=10 || receiver_input_channel_1<=1023)
  {channel_number_ok=1;
  }
  else
  {channel_number_ok=0;}
 ...
 if (channel_number==2 )
 {
  if(receiver_input_channel_2>=10 || receiver_input_channel_2<=1023)
  {channel_number_ok=1;}
  else
  {channel_number_ok=0;}
 }
 if (channel_number==3 )
 {
  if(receiver_input_channel_3>=500 || receiver_input_channel_3<=1023)
  {channel_number_ok=1;}
  else
  {channel_number_ok=0;}
 }
 if (channel_number==4 )
 {
  if(receiver_input_channel_4>=10 || receiver_input_channel_4<=1023)
  {channel_number_ok=1;}
  else
  {channel_number_ok=0;}
 }
...
void check_and_continue()
{

 byte continue_byte = 0;
 while(continue_byte == 0){
       receive_input_NRF();
       if(receiver_input_channel_2 <=100)continue_byte = 1;
       delay(100);
 }
 wait_sticks_zero();
}
//to be removed later as per need
...
void wait_for_receiver(){
  ...
  while(timer > millis() && zero < 15){
    receive_input_NRF();
    if(receiver_input_channel_1 < 255 && receiver_input_channel_1 > 0)zero |= 0b00000001;
    if(receiver_input_channel_2 < 255 && receiver_input_channel_2 > 0)zero |= 0b00000010;
    if(receiver_input_channel_3 < 255 && receiver_input_channel_3 > 0)zero |= 0b00000100;
    if(receiver_input_channel_4 < 255 && receiver_input_channel_4 > 0)zero |= 0b00001000;
    delay(500);
    Serial.print(F("."));
  }
  if(zero == 0){
...
//The following function should be used when using standard 2.4Ghz controller
...
*/
//function to take receiver input data from the controller using NRF module and storing them in variables
...
Command_Val = buf[4]; // Store the new command value, not sure of its functionality as of now
...
void intro(){
  ...
  delay(1500);
  Serial.println(F(""));
  Serial.println(F("AcroBird V1"));
  delay(500);
  Serial.println(F("   By    "));
  delay(500);
  Serial.println(F(" Samannoy Ghosh   "));
  delay(500);
  Serial.println(F("      &         "));
  delay(500);
  Serial.println(F("  Ritam Ghosh    "));
  delay(1000);
  Serial.println(F(""));
  Serial.println(F("AcroBird V1 Flight Controller Setup Program"));
  Serial.println(F(""));
  Serial.println(F("==================================================="));
  delay(1500);
  Serial.println(F("Special Thanks to Joop Brokking for his original work: www.brokking.net"));
  Serial.println(F(""));
